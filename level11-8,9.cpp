...
int main(){
    ...
    previewBuilding.setFillColor(sf::Color(0,255,0,150));
    sf::RectangleShape placedBuilding({80.f,80.f});
    placedBuilding.setFillColor(sf::Color::Blue);
    placedBuilding.setPosition({1000.f,1000.f});
    ...
    sf::Clock clock;
     while(window.isOpen()){
        ...
        //  std::cout<<"x-world"<<" "<<worldMouse.x<<"\n";
        //  std::cout<<"y-world"<<" "<<worldMouse.y<<"\n";
        while(const std::optional event = window.pollEvent()){
            if(event->is<sf::Event::Closed>()){
                window.close();
            }
            if(const auto* mousePressed = event->getIf<sf::Event::MouseButtonPressed>()){
                if(mousePressed->button == sf::Mouse::Button::Left and enemy.getGlobalBounds().contains(worldMouse)){
                    enemySelected = true;
                }
                else{
                    enemySelected=false;
            ...
            if(const auto* mousePressed = event->getIf<sf::Event::MouseButtonPressed>()){
            if(mousePressed->button == sf::Mouse::Button::Left && placeBuilding && canPlace){
                placedBuilding.setPosition(previewBuilding.getPosition());
                buildingPlaced = true;
                placeBuilding =false;
            }
        }
        ...
        if(placeBuilding){
            ...
            if(previewBuilding.getGlobalBounds().findIntersection(enemy.getGlobalBounds())){
            canPlace=false;
        }
        else{
            canPlace=true;
